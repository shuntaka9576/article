...
逆パターンはよくあるそう。Cloudflareはfreeだとネームサーバー設定が変更できないので移管しました。卸価格だから、このサービスモデルを維持する条件として、Cloudflareの提供するDNS（ネームサーバー）を利用させたいんだと思う。サブドメインのみを権限委譲（DNS Delegation）する方法はあるので、取っちゃった人はこれで良いと思う。

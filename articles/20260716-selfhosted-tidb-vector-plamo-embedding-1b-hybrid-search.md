...
体感差がどこから出るかは、検索単体ではなくE2Eで見るのが実態に近いです。E2Eの支配項はクエリ埋め込みの765msなので、1万チャンクでは経路差(最大約170ms)が埋もれます。しかし全件スキャンが行数に線形に伸びるのに対し、ANNはHNSWのグラフ探索が行数に対しておおむね対数的にしか伸びず数十ms級にとどまるため、10倍の10万チャンクではTiFlash全件スキャンのE2Eが約1.5秒とANN(約0.8秒)の2倍近くになって差が体感でき、100倍の100万チャンクでは検索そのものが支配項になって差は決定的です。逆に言えば、E2Eで2秒弱まで許容できるなら、5万記事(チャンク換算で約12万、TiFlash全件スキャンの単純比例で約0.9秒)あたりまでは、取りこぼしのない全件スキャンのまま逃げ切れる計算です。なお、この逃げ切りは列指向のTiFlash全件スキャンが前提で、TiKV全件スキャンは同規模で約2.3秒かかるため厳しくなります。

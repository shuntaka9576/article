...
大容量モデルをローカルで動かす例として、DGX Spark 2台でDeepSeek-V4-Flash-DSparkを動かした報告があります。この構成では重みだけで約167GBあり、1台の128GBには収まりません。そこで2台をConnectX-7経由で接続し、vLLMでモデルを分割して実行しています。共有メモリは合計256GBで、報告された実行中の使用量は1台あたり115〜117GB。重み以外にも、KVキャッシュなどの作業領域が必要です。[2台のSparkでの実行例](https://route179.dev/2026/07/28/deepseek-v4-flash-dual-dgx-spark-eks-hybrid/)

購入費と、クラウドで近いメモリ容量を借りる料金を並べると、こんな違いになります。

- **DGX Spark 2台**：共有メモリ合計256GB。本体は米国参考価格で **$9,398 → 約144.7万円**。税・送料・接続ケーブル・電気代などは別です。
...
実行時には、推論エンジンがモデルに必要な演算を、対象ハードウェア向けに用意した処理へ振り分けます。たとえばllama.cppには、NVIDIA GPU向けのCUDA、AMD GPU向けのHIP、Apple GPU向けのMetalなどの実装があり、CPUだけで計算することもできます。モデルの構造や量子化形式に対応する実装があれば、異なるハードウェアでも同じモデルを実行できます。ただし、数値精度や演算順序による結果の差、処理速度の差はあります。[llama.cppの対応実装](https://github.com/ggml-org/llama.cpp#description)

記事で挙げたGMKtecのEVOやRyzen AI Haloも、AMD製GPUとROCm/HIPなどの組み合わせで推論を実行する端末です。ROCmは科学技術計算も対象としているため、「科学技術計算への対応を省いたからCUDAなしで動く・効率がよい」とは説明できません。効率を比較するには、実際に使う演算の対応状況、メモリ帯域、消費電力、推論速度を見る必要があります。[AMD ROCm](https://www.amd.com/en/products/software/rocm.html)
